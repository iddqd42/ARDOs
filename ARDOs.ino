/* ArDOs   v1.063.5_beta    без режима сна
***Дозиметр на Ардуино
***IDE Arduino 1.8.2
...
#define k_delitel 140 //коефициент дельтеля напряжения, зависит от вашего делителя.
...
void OK () { //нажатие ОК
  if (!(PIND & (1 << PIND3))) { //удержаиние OK
    val_ok++;
//    if (val_ok == 10) {
    if (val_ok == 2) {	//пришлось уменьшить задержку с 10 до 2 циклов, не у всех досчитывает до нужного до момента ежесекундного сброса в функции poisk_f()
      val_ok = 0;
      menu = 2;
    }
  }
